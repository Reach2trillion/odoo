    ...
    def _compute_payment_status(self):
        ...
        for order in self:
            ...
            if not posted_invoices:
                ...
                continue
            payment_states = set(posted_invoices.mapped('payment_state'))
            if payment_states == {'reversed'}:
                order.payment_status = 'Reversed'
            elif payment_states.issubset({'paid', 'in_payment', 'reversed'}):
                # Every posted invoice is settled: 'In Payment' while at
                # least one payment is not yet reconciled with a bank
                # statement, 'Paid' otherwise.
                order.payment_status = 'In Payment' \
                    if 'in_payment' in payment_states else 'Paid'
            elif all(invoice.currency_id.is_zero(
