## Module <payment_status_in_sale>

#### 01.01.2025
#### Version 18.0.1.0.0
##### ADD
- Initial commit for Sale Order Payment Status

#### 25.06.2025
#### Version 18.0.1.0.1
##### ADD
- Bug fixed to ensure the correct payment status is reflected when credit notes are created.

#### 15.07.2025
#### Version 18.0.1.0.2
##### ADD
- Issue resolved in computing the amount due by considering only posted invoices.

#### 20.09.2025
#### Version 18.0.1.0.3
##### ADD
- Issue resolved while clicking view from payment info.

#### 09.12.2025
#### Version 18.0.1.0.4
##### BUG_FIX
- Resolved the issue where the badge was not appearing as “NOT PAID”.

#### 13.08.2026
#### Version 18.0.1.0.5
##### BUG_FIX
- Resolved the issue where the payment status showed 'No invoice' even though posted invoices exist and are paid or in payment (mixed Paid/In Payment invoices, reversed invoices, or a settled down payment invoice alongside an open final invoice).
